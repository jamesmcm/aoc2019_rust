...
pub fn solve_part2(input: &[char]) -> String {
  solve_part2_inner(input, 25, 6).iter().fold(String::from(""), |x, y| x + "\n" + y)
}
...
fn solve_part2_inner(input: &[char], width: usize, height: usize) -> Vec<String> {
    ...
    for chunk in chunks {
      output = output.chars().zip(chunk).map(|x| {
        match x {
          ('2', z) => *z,
          z => z.0,
          }
...
mod tests {
    ...
    fn test_inner1() {
        assert_eq!(
            solve_part1_inner(&input_generator(
                "123456789012"
            ), 3, 2),
            1
        );
    ...
    fn test_inner2() {
        assert_eq!(
            solve_part2_inner(&input_generator(
                "0222112222120000"
            ), 2, 2),
            vec!["01", "10"]
        );
